...
def format_track_list(tracks, header=None):
    ...
    for track in tracks:
        if track is None:
            ...
            message += "«{title}» by {artist} — [{id}] ⚖{weight:2} ♺{count}".format(
                id=track.get('id', 0),
            ...
            if 'labels' in track:
                message += ' #' + ' #'.join(filter_labels(track['labels']))
            message += '\n'
    return message
...
def on_skip(args, sender):
    ...
    if track:
        message = "%s skipped track %u: \"%s\" by %s" % (
            sender_name, track["id"], track["title"], track["artist"])
    else:
        message = "%s skipped an unknown track." % sender_name
...
def on_sql(args, sender):
    ...
    if not args.strip():
        return "This command lets you perform a low-level SQL query and see the results, e.g.: sql SELECT COUNT(*) FROM tracks;"

    if not args.endswith(';'):
        return 'SQL statements must end with a ;, for your own safety.'

    if not args.lower().startswith("select"):
        return "%u rows affected." % ardj.database.execute(args)
...
def on_speak(args, sender):
    return ardj.speech.render_and_queue(
        args) or 'OK, please wait until the current song finishes playing.'
...
def on_ban(args, sender):
    ...
    count = ardj.database.fetchone(
        'SELECT COUNT(*) FROM tracks WHERE artist = ? AND weight > 0', (args, ))[0]
    if not count:
        return 'No tracks by this artist.'
    ardj.database.execute(
        'UPDATE tracks SET weight = 0 WHERE artist = ?', (args, ))
    ardj.jabber.chat_say(
        "%s deleted all %u tracks by %s." %
        (shortname(sender), count, args))
    return 'Deleted %u tracks.' % count


def on_shitlist(args, sender):
    rows = ardj.database.fetch(
        'SELECT id, artist, title, weight, count FROM tracks WHERE weight > 0 ORDER BY weight, title, artist LIMIT 10')
    if not rows:
        return 'No tracks (database must be empty).'
    tracks = [{'id': row[0], 'artist': row[1], 'title': row[2],
               'weight': row[3], 'count': row[4]} for row in rows]
    return format_track_list(tracks, 'Lowest rated tracks:')


def on_hitlist(args, sender):
    rows = ardj.database.fetch(
        'SELECT id, artist, title, weight, count FROM tracks WHERE weight > 0 ORDER BY weight DESC, title, artist LIMIT 10')
    if not rows:
        return 'No tracks (database must be empty).'
    tracks = [{'id': row[0], 'artist': row[1], 'title': row[2],
               'weight': row[3], 'count': row[4]} for row in rows]
    return format_track_list(tracks, 'Highest rated tracks:')
...
def on_queue(args, sender):
    ...
    if args == 'flush -a' and is_admin:
        ...
        if delete:
            for track_id in tracks:
                ardj.database.execute(
                    'DELETE FROM queue WHERE owner = ? AND track_id = ?', (sender, track_id, ))
            return 'Done.'

        tracks = tracks[:1]
        have_tracks = ardj.database.fetchone(
            'SELECT COUNT(*) FROM queue WHERE owner = ?', (sender, ))[0]
        ...
        ardj.jabber.chat_say('%s requested track %s' % (sender.split(
            '@')[0], ', '.join([ardj.tracks.identify(x, unknown='(a pause)') for x in tracks])))
    ...
    return format_track_list(tracks, 'Current queue:')
...
def on_find(args, sender):
    ...
    if len(all_tracks) > len(tracks):
        header = 'Found %u tracks, showing %u:' % (
            len(all_tracks), len(tracks))
    else:
        header = 'Found only these tracks:'
    return format_track_list(tracks, header)


def on_news(args, sender):
    rows = ardj.database.fetch(
        'SELECT id, artist, title, weight, count FROM tracks WHERE weight > 0 ORDER BY id DESC LIMIT 10')
    if not rows:
...
def on_votes(args, sender):
    ...
    pro = [e for e, v in list(votes.items()) if v > 0]
    contra = [e for e, v in list(votes.items()) if v < 0]
    return 'Pro: %s, contra: %s. ' % (
        ', '.join(pro or ['nobody']), ', '.join(contra or ['nobody']))
...
def on_play(args, sender):
    ...
    if not args:
        ...
        return 'Current filter: %s' % ' '.join(current)
...
def on_tags(args, sender):
    ...
    if not args or args == '-a':
        ...
        output = ', '.join(['%s (%u)' % (l, c) for l, c in data]) + '.'
        return output
    ...
    for track_id in track_ids:
        current = ardj.tracks.add_labels(
            track_id, labels, owner=sender) or ['none']

    if len(track_ids) == 1:
        return 'New labels: %s.' % (', '.join(sorted_tags(current)))
    else:
...
def on_last(args, sender):
    rows = ardj.database.fetch(
        'SELECT t.id, t.artist, t.title, t.weight, t.count FROM tracks t INNER JOIN playlog l ON l.track_id = t.id ORDER BY l.ts DESC LIMIT 10')
    tracks = [{'id': row[0], 'artist': row[1], 'title': row[2],
               'weight': row[3], 'count': row[4]} for row in rows]
...
def on_dump(args, sender):
    ...
    votes = ardj.database.fetchone(
        'SELECT vote FROM votes WHERE track_id = ? AND email = ?',
        (track['id'],
         sender,
         ))
...
def on_show(args, sender):
    ...
    result = '«%s» by %s' % (track['title'], track['artist'])
    result += '; id=%u weight=%.2f playcount=%u length=%s vote=%u' % (track['id'],
                                                                      track['weight'] or 0,
                                                                      track['count'] or 0,
                                                                      ardj.util.format_duration(int(track.get('length',
                                                                                                              0))),
                                                                      ardj.tracks.get_vote(track['id'],
                                                                                           sender))
    if track.get("last_played"):
        result += " last_played=%s" % ardj.util.format_duration(
            track['last_played'], age=True)
    result += ". "

    if track['labels']:
        result += 'Labels: ' + \
            ', '.join(sorted_tags(list(set(track['labels'])))) + '. '
    return result.strip()
...
def on_status(args, sender):
    ...
    message = '«%s» by %s — [%u] ♺%u ⚖%.2f Σ%u' % (
        title, artist, track['id'], count, weight, lcount)
    if 'labels' in track:
        message += ' #' + ' #'.join(filter_labels(track['labels']))
    return message
...
def on_admins(args, sender):
    ...
    return "Current admins: %s." % ", ".join(admins)
...
def on_bookmark(args, sender):
    ...
    return 'Bookmark %s. Use "find -b" or "queue -b" to access yout bookmarks.' % (
...
def get_public_commands():
    ...
    public = ardj.settings.get2(
        'jabber/public_commands',
        "public_jabber_commands",
        None)
    if not public:
...
def process_command(text, sender=None, quiet=False):
    ...
    if len(options) > 1:
        return 'Did you mean %s?' % ardj.util.shortlist(
            [o[0] for o in options], limit=1000, glue='or')
