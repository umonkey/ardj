 ...
 *
 * $Id: ices_config.c 7409 2004-07-29 01:17:26Z brendan $
 */
...
parse_file (const char *configfile, ices_config_t *ices_config)
{
  ...
  if (!cur->name || (strcmp ((const char *)cur->name, "Configuration"))) {
  ...
  for (; cur != NULL; cur = cur->next) {
    if (cur->type == XML_COMMENT_NODE)
      continue;
    if (strcmp ((const char *)cur->name, "Stream") == 0) {
    ...
    } else if (strcmp ((const char *)cur->name, "Execution") == 0) {
			     ...
			     ices_stream_t *stream)
{
  for (; cur; cur = cur->next) {
	...
	ices_setup_shutdown ();
      ...
      stream->reencode = res;
    } else if (strcmp ((const char *)cur->name, "Samplerate") == 0) {
      stream->out_samplerate = atoi (ices_xml_read_node (doc, cur));
    } else if (strcmp ((const char *)cur->name, "Channels") == 0) {
		   ...
		   ices_stream_t *ices_config)
{
  for (; cur; cur = cur->next) {
    ...
    if (strcmp ((const char *)cur->name, "Port") == 0) {
      ices_config->port = atoi (ices_xml_read_node (doc, cur));
    } else if (strcmp ((const char *)cur->name, "Hostname") == 0) {
    ...
    } else if (strcmp ((const char *)cur->name, "Protocol") == 0) {
      char *str = ices_xml_read_node (doc, cur);
				...
				ices_config_t *ices_config)
{
  for (; cur; cur = cur->next) {
    ...
    } else if (strcmp ((const char *)cur->name, "Verbose") == 0) {
      ices_config->verbose = atoi (ices_xml_read_node (doc, cur));
    } else if (strcmp ((const char *)cur->name, "BaseDirectory") == 0) {
...
parse_playlist_node (xmlDocPtr doc, xmlNsPtr ns, xmlNodePtr cur, ices_config_t *ices_config)
{
  ...
  for (; cur; cur = cur->next) {
    ...
    if (strcmp ((const char *)cur->name, "Randomize") == 0) {
    ...
    } else if (strcmp ((const char *)cur->name, "File") == 0) {
      ices_util_free (ices_config->pm.playlist_file);
      ices_config->pm.playlist_file =
	ices_util_strdup (ices_xml_read_node (doc, cur));
    } else if (strcmp ((const char *)cur->name, "Module") == 0) {
      ices_util_free (ices_config->pm.module);
      ices_config->pm.module =
	ices_util_strdup (ices_xml_read_node (doc, cur));
    } else if (strcmp ((const char *)cur->name, "Crossfade") == 0) {
