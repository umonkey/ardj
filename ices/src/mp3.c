...
static int ices_mp3_parse (input_stream_t* source)
{
  ...
  if (! strncmp ("OggS", (const char *)mp3_data->buf, 4))
    return 1;

  /* first check for ID3v2 */
  if (! strncmp ("ID3", (const char *)mp3_data->buf, 3))
  ...
  do {
    ...
    if (len < 4) {
      unsigned char* buffer = mp3_data->buf;
...
static int mp3_check_vbr(input_stream_t* source, mp3_header_t* header) {
  ...
  offset += mp3_data->pos;
  if (!strncmp("VBRI", (const char *)mp3_data->buf + offset, 4)
      || !strncmp("Xing", (const char *)mp3_data->buf + offset, 4)) {
