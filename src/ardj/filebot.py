...
class FileBot(JabberBot):
	...
	def si_req_handler(self, conn, mess):
		...
		try:
			...
			for field in si.getTag('feature').getTag('x').getTags('field'):
				if field.attrs['var'] == 'stream-method' and field.getTag('option').getTagData('value') == xmpp.NS_BYTESTREAM:
					transfer = {
					...
					if self.is_file_acceptable(transfer['from'], transfer['name'], transfer['size']):
						self.transfers[mess.attrs['id']] = transfer
					...
					#	reply.addChild('error', { 'type': 'auth' }) \
					#		.addChild('not-acceptable', namespace=xmpp.NS_STANZAS)
					#	self.log('Refusing to accept %s from %s.' % (transfer['name'], transfer['from']))
	...
	def on_bytestream(self, conn, mess):
		...
		try:
			...
			if self.transfers.has_key(sid):
				transfer = self.transfers[sid]
				self.debug('Sending %s' % transfer)
		...
		for sid in self.transfers.keys():
			if self.transfers[sid].has_key('socket'):
				...
				if got:
					...
					self.debug('God %u bytes from socket %s, %u left.' % (got, transfer['socket'].fileno(), transfer['size'] - transfer['received']))
