...
class Status(object):
    ...
    def NewFromJsonDict(data):
        ...
        return Status(created_at=data.get('created_at', None),
                      ...
                      in_reply_to_screen_name=data.get(
                          'in_reply_to_screen_name', None),
                      in_reply_to_user_id=data.get(
                          'in_reply_to_user_id', None),
                      in_reply_to_status_id=data.get(
                          'in_reply_to_status_id', None),
...
class Api(object):
    ...
    def GetSearch(self,
        ...
        parameters['q'] = urllib.parse.quote_plus(term)
    ...
    def GetFriendsTimeline(self,
        ...
        if not user and not self._oauth_consumer:
            raise TwitterError(
                "User must be specified if API is not authenticated.")
    ...
    def GetUserTimeline(self,
        ...
        if id:
            ...
            url = '%s/statuses/user_timeline.json?user_id=%d' % (
    ...
    def VerifyCredentials(self):
        ...
        if not self._oauth_consumer:
            raise TwitterError(
                "Api instance must first be given user credentials.")
    ...
    def MaximumHitFrequency(self):
        ...
        if reset_time and limit:
            ...
            # hour
            delta = reset + \
                datetime.timedelta(hours=1) - datetime.datetime.utcnow()
    ...
    def _EncodeParameters(self, parameters):
        ...
        if parameters is None:
            ...
            return urllib.parse.urlencode(dict(
                [(k, self._Encode(v)) for k, v in list(parameters.items()) if v is not None]))
    ...
    def _EncodePostData(self, post_data):
        ...
        if post_data is None:
            ...
            return urllib.parse.urlencode(
                dict([(k, self._Encode(v)) for k, v in list(post_data.items())]))
    ...
    def _FetchUrl(self,
        ...
        if self._oauth_consumer is not None:
            ...
            req.sign_request(
                self._signature_method_hmac_sha1,
        ...
        if encoded_post_data or no_cache or not self._cache or not self._cache_timeout:
            ...
            # Unique keys are a combination of the url and the oAuth Consumer
            # Key
            if self._username:
...
def get_replies():
    ...
    return [(s.user.screen_name, s.text) for s in sorted(
        get_client().GetReplies(), key=lambda s: s.created_at_in_seconds, reverse=True)]
...
def speak_message(author, message, play=False, queue=True):
    ...
    if update:
        tmpname, duration = ardj.speech.render_text(
            message, artist='@' + author, title=message)
        if not duration:
...
def send_status_net(message):
    ...
    response_url = ardj.settings.get(
        "status_net_response_url",
        "http://identi.ca/notice/%s")
...
def send_message(message):
    ...
    return 'http://twitter.com/' + posting.GetUser().GetScreenName() + \
        '/status/' + str(posting.GetId())
