...
class ardjbot(MyFileReceivingBot):
    ...
    def show(self, message, args):
        ...
        result += u'; id=%u weight=%.2f playcount=%u length=%s vote=%u last_played=%s. ' % (track['id'], track['weight'] or 0, track['count'] or 0, ardj.util.format_duration(int(track.get('length', 0))), ardj.tracks.get_vote(track['id'], message.getFrom().getStripped()), ardj.util.format_duration(track.get('last_played', 0), age=True))
        if track['labels']:
