...
class SamplePlaylist(object):
    ...
    def remove_missing_files(self, playlist):
        return list(filter(os.path.exists, playlist))
...
class Playlist(dict):
    ...
    def is_active(self, timestamp=None):
        ...
        if 'delay' in self and self['delay'] * \
                60 + self['last_played'] >= now_ts:
            return False
    ...
    def get_all(cls):
        ...
        `playlists' table."""
        stats = dict(ardj.database.fetch(
            'SELECT name, last_played FROM playlists WHERE name IS NOT NULL AND last_played IS NOT NULL'))
        return [cls(p).add_ts(stats)
                for p in ardj.settings.load().get_playlists()]
            ...
            if name and playlist.match_track(track):
                ...
                if rowcount == 0:
                    ardj.database.execute(
                        'INSERT INTO playlists (name, last_played) VALUES (?, ?)', (name, ts, ))
...
class Track(dict):
    ...
    def get_labels(self):
        if "labels" not in self:
            rows = ardj.database.fetch(
                "SELECT label FROM labels WHERE track_id = ?", (self["id"], ))
    ...
    def get_by_id(cls, track_id):
        ...
        sql = "SELECT %s FROM %s WHERE %s = ?" % (
            ", ".join(cls.fields), cls.table_name, cls.key_name)
    ...
    @classmethod
    def get_last(cls):
        sql = "SELECT %s FROM %s ORDER BY last_played DESC LIMIT 1" % (
            ", ".join(cls.fields), cls.table_name)
        row = ardj.database.fetchone(sql)
        return Track([(cls.fields[k], v) for k, v in enumerate(row)])

    def get_last_vote(self, sender):
        votes = ardj.database.fetchone(
            "SELECT vote FROM votes WHERE track_id = ? AND email = ? ORDER BY track_id DESC",
            (self["id"],
             sender,
             ))
    ...
    def refresh_tags(self, filepath):
        ...
        if duration != self["length"]:
            log_info("Updating track {0} duration to {1}.",
                     self["id"], duration)
        ...
        if artist != self["artist"]:
            log_info("Updating track {0} artist to {1}.",
                     self["id"], artist)
            self["artist"] = artist
            write = True
        ...
        if title != self["title"]:
            log_info("Updating track {0} title to {1}.",
                     self["id"], title)
            self["title"] = title
            write = True
        ...
        if write:
            ardj.database.execute(
                "UPDATE tracks SET artist = ?, title = ?, length = ? WHERE id = ?",
                (artist,
                 title,
                 duration,
                 self["id"]))
...
def get_last_track_id():
    ...
    """
    row = ardj.database.fetchone(
        "SELECT id FROM tracks ORDER BY last_played DESC LIMIT 1")
    if row:
        return row[0]
...
def identify(track_id, unknown='an unknown track'):
    ...
    if not track:
        return unknown
    return '«%s» by %s [%u]' % (track.get('title', 'untitled'), track.get(
        'artist', 'unknown artist'), track_id)


def queue(track_id, owner=None):
    """Adds the track to queue."""
    return ardj.database.execute(
        'INSERT INTO queue (track_id, owner) VALUES (?, ?)', (track_id, (owner or 'ardj').lower(), ))
...
def find_ids(pattern, sender=None, limit=None):
    ...
    sql = 'SELECT id FROM tracks WHERE weight > 0 AND %s ORDER BY %s' % (
        ' AND '.join(where), search_order)
...
def _add_label_filter(labels, where, params):
    ...
    for label in labels:
        if label.startswith("+"):
            ...
            where.append(
                'id NOT IN (SELECT track_id FROM labels WHERE label = ?)')
            params.append(label[1:])
        else:
            other_labels.append(label)

    if other_labels:
        sql = "id IN (SELECT track_id FROM labels WHERE label IN (%s))" % ", ".join(
            ['?'] * len(other_labels))
        where.append(sql)
...
def add_labels(track_id, labels, owner=None):
    if labels:
        for label in labels:
            if label.startswith('-'):
                ardj.database.execute(
                    'DELETE FROM labels WHERE track_id = ? AND label = ?',
                    (track_id,
                     label.lstrip('-'),
                     ))
...
def update_track(properties):
    ...
    params = []
    for k in properties:
        if k in ('filename', 'artist', 'title', 'length', 'weight', 'count',
                 'last_played', 'owner', 'real_weight', 'download', 'image'):
            sql.append(k + ' = ?')
...
def purge():
    ...
    for track_id, filename in ardj.database.fetch(
            'SELECT id, filename FROM tracks WHERE weight > 0 AND filename IS NOT NULL'):
        abs_filename = os.path.join(music_dir, filename)
        if not os.path.exists(abs_filename):
            logging.warning(
                'Track %u vanished (%s), deleting.' %
                (track_id, filename))
            ardj.database.execute(
                'UPDATE tracks SET weight = 0 WHERE id = ?', (track_id, ))
    ...
    for track_id, filename in ardj.database.fetch(
        ...
        if os.path.exists(abs_filename):
            ...
            logging.info(
                'Deleted track %u (%s) from file system.' %
                (track_id, filename))
...
def get_urgent():
    ...
    data = ardj.database.fetch(
        'SELECT labels FROM urgent_playlists WHERE expires > ? ORDER BY expires',
        (int(
            time.time()),
         ))
    if data:
...
def set_urgent(args):
    ...
    if args == 'all':
        ardj.jabber.chat_say("Returning to normal playlists.")
    else:
        duration, args = extract_duration(args)
        expires = time.time() + duration * 60
        ardj.database.execute(
            'INSERT INTO urgent_playlists (labels, expires) VALUES (?, ?)',
            (args,
             int(expires),
             ))
        ardj.jabber.chat_say(
            "Playlist for next %u minutes: %s." %
            (duration, args))
...
def add_vote(track_id, email, vote, update_karma=False):
    ...
    row = ardj.database.fetchone(
        "SELECT last_played, weight FROM tracks WHERE id = ?", (track_id, ))
    if row is None:
        return None
    ...
    if not vote_count:
        ...
        ardj.database.execute(
            'UPDATE tracks SET weight = ? WHERE id = ?', (current_weight, track_id, ))
...
def get_track_votes(track_id):
    results = {}
    for email, vote in ardj.database.fetch(
            "SELECT email, vote FROM votes WHERE track_id = ?", (track_id, )):
        results[email.lower()] = vote
    return results


def add_file(filename, add_labels=None, owner=None,
             quiet=False, artist=None, title=None, dlink=None):
    ...
    track_id = ardj.database.execute(
        'INSERT INTO tracks (artist, title, filename, length, last_played, owner, weight, real_weight, count, download) VALUES (?, ?, ?, ?, ?, ?, 1, 1, 0, ?)',
        (artist,
         title,
         rel_path,
         duration,
...
def get_track_id_from_queue():
    ...
    """
    row = ardj.database.fetchone(
        'SELECT id, track_id FROM queue ORDER BY id LIMIT 1')
    if row:
...
def get_random_track_id_from_playlist(playlist, skip_artists):
    ...
    track_id = get_random_row(
        ardj.database.fetch(
            sql, tuple(params)), playlist.get(
            "strategy", "default"))
    ...
    with Sticky() as sticky:
        ...
        if sticky["playlist"] != pl_name:
            if sticky["label"] is not None:
                logging.info(
                    "Playlist changed to %s, dropping sticky label \"%s\"." %
                    (pl_name, sticky["label"]))
                sticky["label"] = None
        ...
        if not playlist.get("sticky_labels"):
            logging.debug(
                "Sticky: playlist %s has no sticky_labels." %
                sticky["playlist"])
            return
        ...
        if track is None:
            logging.debug(
                "Sticky: track %s not found -- no labels to pick from." %
                track_id)
            return
        ...
        if not labels:
            logging.debug(
                "Sticky: track %u has no labels to stick to playlist %s." %
                (track["id"], sticky["playlist"]))
            return
...
def get_sticky_label(playlist):
    ...
    with Sticky() as sticky:
        ...
        if playlist.get("name", "unnamed") != sticky["playlist"]:
            if sticky["label"]:
                logging.debug(
                    "Sticky: playlist changed to %s, forgetting old label %s." %
                    (playlist.get("name"), sticky["label"]))
            return []
...
def add_labels_filter(sql, params, labels):
    either = [l for l in labels if not l.startswith(
        '-') and not l.startswith('+')]
    neither = [l[1:] for l in labels if l.startswith('-')]
    every = [l[1:] for l in labels if l.startswith('+')]

    if either:
        sql += ' AND id IN (SELECT track_id FROM labels WHERE label IN (%s))' % ', '.join(
            ['?'] * len(either))
        params += either

    if neither:
        sql += ' AND id NOT IN (SELECT track_id FROM labels WHERE label IN (%s))' % ', '.join(
            ['?'] * len(neither))
        params += neither
...
def get_random_row(rows, strategy=None):
    ...
    logging.debug(
        "Picked track %s using strategy '%s'." %
        (track_id, strategy))
    return track_id
...
def get_prerolls_for_labels(labels):
    ...
    sql = "SELECT tracks.id FROM tracks INNER JOIN labels ON labels.track_id = tracks.id WHERE tracks.weight > 0 AND tracks.filename IS NOT NULL AND labels.label IN (%s)" % ', '.join([
        '?'] * len(labels))
...
def get_prerolls_for_track(track_id):
    ...
    by_artist = ardj.database.fetch(
        "SELECT t1.id FROM tracks t1 INNER JOIN tracks t2 ON t2.artist = t1.artist INNER JOIN labels l ON l.track_id = t1.id WHERE l.label = 'preroll' AND t2.id = ? AND t1.weight > 0 AND t1.filename IS NOT NULL",
        (track_id,
         ))
    by_label = ardj.database.fetch(
        "SELECT t.id, t.title FROM tracks t WHERE t.weight > 0 AND t.filename IS NOT NULL AND t.id IN (SELECT track_id FROM labels WHERE label IN (SELECT l.label || '-preroll' FROM tracks t1 INNER JOIN labels l ON l.track_id = t1.id WHERE t1.id = ?))",
        (track_id,
         ))
...
def add_preroll(track_id, labels=None):
    ...
    logging.debug(
        "Looking for prerolls for track %u (labels=%s)" %
        (track_id, labels))

    row = ardj.database.fetchone(
        "SELECT COUNT(*) FROM labels WHERE track_id = ? AND label LIKE 'preroll-%'",
        (track_id,
         ))
    if row and row[0]:
...
def get_next_track():
    try:
        ...
        if not track_id:
            logging.warning(
                "Could not find a track to play -- empty database?")
            return None
...
def get_next_track_id(update_stats=True):
    ...
    if dupe_count:
        skip_artists = list(set([row[0] for row in ardj.database.fetch(
            'SELECT artist FROM tracks WHERE artist IS NOT NULL AND last_played IS NOT NULL ORDER BY last_played DESC LIMIT ' + str(dupe_count))]))
    else:
        skip_artists = []
    ...
    if not track_id:
        ...
        if labels:
            track_id = get_random_track_id_from_playlist(
                {'labels': labels}, skip_artists)
            if debug and track_id:
                logging.debug(
                    'Picked track %u from the urgent playlist.' %
                    track_id)
    ...
    if not track_id:
        logging.debug(
            "Falling back to just any random track from the database.")

        rows = ardj.database.fetch(
            "SELECT id, weight, artist, count, last_played FROM tracks WHERE weight > 0")
    ...
    if track_id:
        ...
        if update_stats:
            count = ardj.database.fetch(
                'SELECT count FROM tracks WHERE id = ?', (track_id, ))[0][0] or 0
            ardj.database.execute(
                'UPDATE tracks SET count = ?, last_played = ? WHERE id = ?',
                (count + 1,
                 int(
                     time.time()),
                    track_id,
...
def shift_track_weight(track_id):
    ...
    weight, real_weight = ardj.database.fetchone(
        "SELECT weight, real_weight FROM tracks WHERE id = ?", (track_id, ))
    if weight < real_weight:
        weight = min(weight + 0.1, real_weight)
    elif weight > real_weight:
        weight = max(weight - 0.1, real_weight)
    ardj.database.execute(
        "UPDATE tracks SET weight = ? WHERE id = ?", (weight, track_id, ))
...
def log(track_id, listener_count=None, ts=None):
    ...
    ardj.database.execute(
        'INSERT INTO playlog (ts, track_id, listeners) VALUES (?, ?, ?)',
        (int(
            ts or time.time()),
            int(track_id),
            listener_count,
...
def update_real_track_weight(track_id):
    ...
    ardj.database.execute(
        'UPDATE tracks SET real_weight = ? WHERE id = ?', (real_weight, track_id, ))
    return real_weight
...
def update_karma():
    ...
    for email, ts in ardj.database.fetch(
            'SELECT email, MAX(ts) FROM votes GROUP BY email'):
        diff = now - ts / 86400
        if diff == 0:
            karma = 1
        elif diff > KARMA_TTL:
        ...
        if karma:
            ardj.database.execute(
                'INSERT INTO karma (email, weight) VALUES (?, ?)', (email, karma, ))
...
def merge(id1, id2):
    ...
    ardj.database.execute(
        'UPDATE labels SET track_id = ? WHERE track_id = ?', (id1, id2, ))
    ardj.database.execute(
        'UPDATE votes SET track_id = ? WHERE track_id = ?', (id1, id2, ))
    ardj.database.execute(
        'UPDATE playlog SET track_id = ? WHERE track_id = ?', (id1, id2, ))
...
def update_track_lengths(only_ids=None):
    ...
    for length, id in updates:
        ardj.database.execute(
            'UPDATE tracks SET length = ? WHERE id = ?', (length, id, ))
...
def bookmark(track_ids, owner, remove=False):
    ...
    for track_id in track_ids:
        ardj.database.execute(
            'DELETE FROM labels WHERE track_id = ? AND label = ?', (track_id, label, ))
        if not remove:
            ardj.database.execute(
                'INSERT INTO labels (track_id, label, email) VALUES (?, ?, ?)',
                (track_id,
...
def find_by_artist(artist_name):
    rows = ardj.database.fetch(
        'SELECT id FROM tracks WHERE artist = ? COLLATE unicode', (artist_name, ))
    return [row[0] for row in rows]


def find_by_filename(pattern):
    rows = ardj.database.fetch(
        'SELECT id FROM tracks WHERE filename LIKE ? COLLATE unicode', (pattern, ))
    return [row[0] for row in rows]
...
def find_by_title(title, artist_name=None):
    ...
    if artist_name is None:
        rows = ardj.database.fetch(
            'SELECT id FROM tracks WHERE title = ? COLLATE unicode', (title, ))
    else:
        rows = ardj.database.fetch(
            'SELECT id FROM tracks WHERE title = ? COLLATE unicode AND artist = ? COLLATE unicode',
            (title,
             artist_name,
             ))
    return [row[0] for row in rows]
...
def get_new_tracks(artist_names=None, label='music', weight=1.5):
    ...
    return get_missing_tracks(tracklist, limit=ardj.settings.get(
        'fresh_music/tracks_per_artist', 2))
...
def mark_long():
    ...
    ardj.database.execute(
        "INSERT INTO `labels` (`track_id`, `email`, `label`) SELECT id, \'ardj\', ? FROM tracks WHERE length > ?",
        (tag,
         length,
         ))
    count = ardj.database.fetch(
        'SELECT COUNT(*) FROM labels WHERE label = ?', (tag, ))[0][0]
...
def find_new_tracks(args, label='music', weight=1.5):
    ...
    for track in tracks:
        ...
        try:
            ...
            filename = ardj.util.fetch(
                str(track['url']), suffix=track.get('suffix'))
            if not is_dry_run():
    ...
    if added:
        ...
        if not is_dry_run():
            ardj.jabber.chat_say(
                'Downloaded %u new tracks by %s.' %
                (added, ardj.util.shortlist(
                    sorted(artist_names))))
...
def schedule_download(artist, owner=None):
    ...
    count = ardj.database.fetchone(
        'SELECT COUNT (*) FROM tracks WHERE weight > 0 AND artist = ? COLLATE unicode',
        (artist,
         ))
    if count[0]:
        return 'Песни этого исполнителя уже есть.  Новые песни загружаются автоматически в фоновом режиме.'

    ardj.database.execute(
        "INSERT INTO download_queue (artist, owner) VALUES (?, ?)", (artist, owner, ))
    ardj.database.commit()

    return "Это займёт какое-то время, я сообщу о результате."
...
def add_label_to_tracks_liked_by(label, jids, sender):
    ...
    _sets = [set(ardj.database.fetchcol(
        "SELECT track_id FROM votes WHERE email = ? AND vote > 0", (jid, ))) for jid in jids]
    while len(_sets) > 1:
        _sets[0] &= _sets[1]
        del _sets[1]
...
def add_missing_lastfm_tags():
    ...
    for track in sorted(tracks, key=lambda t: t["id"], reverse=True):
        ...
        if skip_labels and set(labels) & skip_labels:
            ...
            except ardj.scrobbler.BadAuth as e:
...
def do_idle_tasks(set_busy=None):
    ...
    req = ardj.database.fetchone(
        "SELECT artist, owner FROM download_queue LIMIT 1")
    if req is None:
        return False
    ...
    if count:
        msg = "Added %u tracks by %s." % (count, artist_name)
        ardj.jabber.chat_say(msg)
    else:
        msg = "Could not find anything by %s on Last.fm and Jamendo." % artist_name
    ardj.jabber.chat_say(msg, recipient=sender)

    ardj.database.execute(
        "DELETE FROM download_queue WHERE artist = ?", (artist_name, ))
    ardj.database.commit()
...
def get_track_to_play_next():
    ...
    if not count:
        logging.warning(
            "There are NO tracks in the database.  Put some files in %s, then run 'ardj tracks scan'." %
            music_dir)
    else:
        logging.warning(
            "Could not pick a track to play.  Details can be found in %s." %
            settings.getpath(
                "log", "syslog"))
